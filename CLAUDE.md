...
**Last updated**: 2026-09-11 (Fri) — automated Friday audit; no game dev since 2026-05-19

### Where we are
**Phases 1 + A–L complete. All IDEAS.md questions resolved. Architecture healthy. Project is in an extended pause.**

`main` is at `0278177` (feat: configurable event monitoring dashboard, 2026-05-19). CI green on all 69 runs. 124 days since last game commit (Phase L, 2026-05-10).

**17 open Friday audit PRs** (#21–#42) have never been merged. They contain RECAPS entries and prior CLAUDE.md updates from each week's automated audit. When you're ready to resume, the cleanest path is: close those stale PRs in batch (they're docs-only and this audit supersedes them), then start fresh from main.

**4 open AUDIT-CRITICAL issues**: #24, #26, #35, #39 — all flagging the same extended-gap condition. No new issue filed this week (redundant). Close them alongside the stale PRs when resuming.

### Test surface
**13 test files green** on `main` (12 from Phase 1 + `test_monster_ai` from Phase J):
- `test_rng_determinism`, `test_game_state_serialization`, `test_attack_command`, `test_command_processor`, `test_end_turn_command`, `test_monster_state`, `test_scripted_fight`, `test_ability_def`, `test_use_ability_command`, `test_replay`, `test_monster_def`, `test_status_effect`, `test_monster_ai`

### What architecture exists (as of main = 0278177)
- **Resources**: `GameState`, `PlayerState`, `MonsterState`, `AbilityDef` (incl. `applies_effects`, save fields), `MonsterDef`, `StatusEffect`, `EventLog`, `RNGService`
- **Commands**: `AttackCommand`, `EndTurnCommand` (status-effect tick + AP-driven monster turns), `UseAbilityCommand` (heal + damage paths, multi-target, save throws), `ApplyStatusEffectCommand`
- **Monster AI**: `MonsterAI` helper; monsters use AP-driven turns via shared `EndTurnCommand` pipeline
- **Content** (`.tres`): `fighter_slash`, `fighter_cleave`, `fighter_power_strike`, `fighter_second_wind`, `fighter_shield_bash` (with save throw); `champion_great_weapon_master`, `champion_critical_finisher`; `skeleton_warrior`, `skeleton_archer`, `zombie`, `ghoul`
- **Effects**: stun, bleed (DOT), poison, slow, regenerate — all data-driven on `StatusEffect`; stacking = refresh-on-same-id
- **Damage types + resistances**: typed damage on `AbilityDef`; per-actor resistance dictionaries on `MonsterState`/`PlayerState`
- **Dashboard**: `dashboard/` — Next.js event monitoring dashboard (configurable, 2026-05-19)
- **Replay**: `EventLog.replay()` round-trips against fresh initial state. Full-fight integration test covers all Commands.

### File size violations (pre-existing, flagged since Phase L)
- `use_ability_command.gd`: 352 lines — over 150-line rule. Candidate for split when Phase 2 work begins.
- `end_turn_command.gd`: 194 lines — marginally over. Monitor.

### What needs to happen first when we resume
1. **Merge or close the 17 open audit PRs** — easiest: close #21–#41 as superseded, then decide whether to merge #42 + this PR (#43) or just close them too after manually applying the RECAPS entry.
2. **Close the 4 open AUDIT-CRITICAL issues** (#24, #26, #35, #39) — the extended gap is acknowledged; they've served their purpose.
3. **Phase 2**: combat scene UI, dice-roll animation, dice-feel polish per `ROADMAP.md`. Architecture is ready. No design questions are currently open.
4. **`use_ability_command.gd` refactor** — 352 lines. Good first task for a return session (low risk, improves long-term health).

### Open admin items (tracked, not blocking)
- LLC formation pending (Month 1-2 per `VIBE_CODING.md`) — still pending
...
- `RECAPS.md` "How I felt" lines are blank on both 2026-05-08 and 2026-05-10 entries
- Dispatch / mobile push setup still partial (per TECH_DEBT.md)

### Working memory worth carrying over
- **`gh pr merge --delete-branch` fails from a worktree**. Workaround: `--delete-branch=false` + `git fetch --prune`. See TECH_DEBT.md.
- **Tests use `preload()` rather than `class_name`**. Robust on fresh checkouts before first `--import`.
...
- **EventLog.replay does NOT re-append to the replayed state's log** — log is input, not output. See `event_log.gd`.
- **5-min cache TTL trap**: prefer ~270s or 1200-1800s waits. 300s is worst-of-both.

### Branch / files involved
- `main` is at `0278177`. All Phase 1 + A–L branches merged.
- This audit lands on `friday-audit-2026-09-11`, PR #43 against main. Do not merge until user reviews.
